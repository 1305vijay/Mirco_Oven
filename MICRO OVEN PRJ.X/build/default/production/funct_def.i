...
unsigned char timer[] = "0000";
unsigned char sec[] = "00", min[] = "00";
static unsigned char micro_once = 1, grill_once = 1, convec_once = 1, start_once = 1;
...
void blink_timer_display(char micro_flag, char micro_pos) {
    ...
    if (micro_flag == 0 && ((micro_pos - 1) == 1) && blink_state)
        ...
        clcd_putch(min[1], (0xC0 + 5));

    if (micro_flag == 0 && ((micro_pos - 1) == 0) && blink_state)
        clcd_putch('_', (0xC0 + 6));
    else
        clcd_putch(min[0], (0xC0 + 6));
    ...
    if (micro_flag == 1 && ((micro_pos - 1) == 1) && blink_state)
        ...
        clcd_putch(sec[1], (0xC0 + 8));

    if (micro_flag == 1 && ((micro_pos - 1) == 0) && blink_state)
        clcd_putch('_', (0xC0 + 9));
    else
        clcd_putch(sec[0], (0xC0 + 9));
}
...
unsigned char convection_mode(unsigned char key)
{
    ...
    if (pre_heat)
    {
        ...
        static unsigned char temp_pos = 0;

    if (key != 0xFF && (key >= 0 && key <= 9))
    {
        unsigned char digit = key ;
    ...
    clcd_putch((temp / 100)+'0', (0xC0 + 6));
    clcd_putch(((temp / 10) % 10) +'0' , (0xC0 + 7));
    clcd_putch((temp % 10)+'0', (0xC0 + 8));
    ...
    if (key == '#')
    {
        ...
        {
            clear_display();
            clcd_print("Invalid Temp!", (0x80 + 0));
        ...
        else
        {
            ...
            while (count >= 0)
            {
                ...
                clcd_putch((count / 10) + '0', (0xC0 + 11));
                clcd_putch((count % 10) + '0', (0xC0 + 12));
...
unsigned char start_operation()
{
    ...
    total_seconds = ((timer[0] - '0') * 600) +
                    ((timer[1] - '0') * 60) +
                    ((timer[2] - '0') * 10) +
                    (timer[3] - '0');


    clcd_putch(timer[0], (0x80 + 9));
    clcd_putch(timer[1], (0x80 + 10));
    clcd_putch(':',(0x80 + 11));
    clcd_putch(timer[2], (0x80 + 12));
    clcd_putch(timer[3], (0x80 + 13));
